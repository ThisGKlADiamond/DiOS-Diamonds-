...
LANGUAGES = {
    'en': {
        ...
        'started': "EXP OS {version} started.",  # Убрана лишняя 'alpha'
    ...
    'fr': {
        ...
        'started': "EXP OS {version} a démarré.",  # Убрана лишняя 'alpha'
    ...
    'nl': {
        ...
        'started': "EXP OS {version} is gestart.",  # Убрана лишняя 'alpha'
...
class SimpleOS:
    def __init__(self):
        self.running = True
        self.version = "alpha 0.0.1 Build 7"
        self.history = []
        self.username = "User"
        self.lang = "en"
    ...
    def cmd_touch(self, args):
        if not args:
            ...
            return
        try:
            # Открываем файл в режиме добавления ('a'), чтобы создать его,
            # если его нет, и не стирать содержимое, если он уже существует.
