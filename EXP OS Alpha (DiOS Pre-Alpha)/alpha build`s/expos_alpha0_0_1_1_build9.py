...
class EXPOS:  # Класс переименован
    def __init__(self):
        self.running = True
        self.is_restarting = False
        self.version = "alpha 0.0.1.1 Build 9"  # Версия обновлена
        self.history = []
        self.username = "User"
        self.lang = "en"
        self.config_file = ".expos_config"

        # Команды shutdown, restart и settings убраны из основного списка
        self.commands = {
            'help': self.cmd_help,
            'menu': self.cmd_menu,  # Добавлено главное меню
            'echo': self.cmd_echo,
            'calc': self.cmd_calc,
    ...
    def help_text(self):
        return {
            ...
            'menu': self.t('desc_menu'),  # Описание меню в help
    ...
    # ===== МЕНЮ НАСТРОЕК (ВЛОЖЕНО В MENU) =====
    def cmd_settings(self, args):
        print(self.t('set_title'))
        print(self.t('set_help'))

        while self.running:  # Проверяем self.running на случай, если ОС выключили
...
if __name__ == "__main__":
    while True:
        my_os = EXPOS()  # Используем новое имя класса
        my_os.start()
