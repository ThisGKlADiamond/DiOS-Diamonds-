...
LANGUAGES = {
    ...
    'fr': {
        ...
        'desc_restart': "Redémarrer le système",
    ...
    'nl': {
        ...
        'desc_restart': "Start het besturingssysteem opnieuw op",
...
class SimpleOS:
    def __init__(self):
        self.running = True
        self.is_restarting = False  # Флаг для определения перезагрузки
        self.version = "alpha 0.0.1"  # Обновлена версия
        self.history = []
        self.username = "User"
        self.lang = "en"
        ...
        self.commands = {
            ...
            'restart': self.cmd_restart,  # Добавлена команда перезапуска
            'calc': self.cmd_calc,
    ...
    def help_text(self):
        return {
            ...
            'restart': self.t('desc_restart'),  # Описание перезапуска
    ...
    def run_sysinfo(self):
        ...
        print(f"EXP OS Version : {self.version} Build 8")
    ...
    def cmd_restart(self, args):
        ...
        # Визуальная имитация перезапуска ПК
        print("BIOS CHECK: OK")
        time.sleep(0.5)
...
if __name__ == "__main__":
    # Внешний цикл для поддержки "чистого" перезапуска
    while True:
        my_os = SimpleOS()
        my_os.start()

        # Если флаг перезапуска не поднят (т.е. был shutdown), выходим из цикла полностью
