...
try:
    ...
    COMMANDS = ['menu', 'help', 'clear', 'settings', 'store', 'files', 'browser', 'calc', 'messages',
                ...
                'profile', 'weather', 'calendar', 'resolution']
...
resolution    : Configure Display & Screen Bounds
...
"""
    ...
    def boot_sequence(self, target_state="os", skip_bios=False):
        ...
        print("Initializing Hardware (Dynamic Display Native)...")
        self.beep(1200, 200)
    ...
    # --- NEW BOOT BANNER METHOD ---
    def display_boot_banner(self):
        """Displays the OS welcome banner before the command loop."""
        self._clear_screen()
        print(f"{THEMES['cyber']}")
        print("==================================================")
        print(f"                 DiOS {self.version}              ")
        print("==================================================")
        print(f"{THEMES['standard']}")
        print("Welcome to DiOS. System booted successfully.")
        print("Type 'help' to see a list of available commands.\n")
        # Pause for user input so the screen doesn't clear instantly
        input(f"{THEMES['cyber']}[Press Enter to start the Desktop]{THEMES['standard']}")
    ...
    def run_install_register(self):
        ...
        self.users_db[name] = {
            ...
            "password": pwd,
            "theme": "cyber",
            "wallpaper": "black",  # DEFAULT WALLPAPER UPDATED
            "taskbar_color": "neon",
            "installed_apps": [],
            "coins": 62,
            "level": 1,
            ...
            "aliases": {},
            "autostart": "",
            "dynamic_resolution": True,  # NEW SETTING MIGRATION
    ...
    def login(self):
        while True:
            ...
            if name in self.users_db and pwd == self.users_db[name]["password"]:
                ...
                self.user_data.setdefault("wallpaper", "black")
                self.user_data.setdefault("dynamic_resolution", True)
                self.update_title()
                ...
                self.user_data = {"name": "root", "coins": 0, "level": 99, "title": "SYSTEM GOD", "installed_apps": [],
                                  "wallpaper": "black", "aliases": {}, "autostart": "", "dynamic_resolution": True}
    ...
    def run_admin_panel(self):
        ...
        while True:
            ...
            if cmd == "home":
                ...
                for i in range(3):
                    print(f"kernel: memory block {hex(i * 2048)} clear")
            elif cmd == "reset":
        ...
        while self.current_state == "edu_os":
            ...
            if cmd == "0":
                ...
                if shop_cmd == "1":
                    ...
                    print("Hint tickets are coming in version 1.4.9! Keep your coins.")
                input("Press Enter...")
    ...
    def show_menu(self):
        ...
        self._print_buf("   - resolution(Screen setup)")
    ...
    def run_resolution(self):
        self.active_app = "Resolution"
        while True:
            self._clear_screen()
            cols, lines = shutil.get_terminal_size(fallback=(80, 24))
            dynamic_status = self.user_data.get("dynamic_resolution", True)

            print(f"{THEMES['cyber']}🖥️ DiOS DISPLAY MANAGER 🖥️{THEMES['standard']}")
            print(f"Detected physical window bounds: {cols} columns x {lines} rows")
            print("-" * 40)
            print(f"DiOS {self.version} supports responsive monitor scaling.")
            print(f"Dynamic Desktop Scaling: {'[ON]' if dynamic_status else '[OFF]'}")
            print("\n[1] Toggle Dynamic Scaling")
            print("[Q] Return to Desktop")

            c = input("> ").strip().lower()
            if c == 'q':
                break
            elif c == '1':
                self.user_data["dynamic_resolution"] = not dynamic_status
                self.save_system()
                self.set_notify(f"Scaling set to: {self.user_data['dynamic_resolution']}")
        self.active_app = None
    ...
    def play_hide_seek(self):
        ...
        if "hide_seek" not in self.user_data.get("installed_apps", []):
            return self._print_buf("Install from Store.")
        try:
            self._clear_screen()
    ...
    def play_number_guess(self):
        ...
        if "number_guess" not in self.user_data.get("installed_apps", []):
            return self._print_buf("Install from Store.")
        self._clear_screen()
    ...
    def run_messages(self):
        ...
        if cmd == '+':
            self.messenger_contacts[str(len(self.messenger_contacts) + 1)] = input("Name: ")
        self.active_app = None

    def run_antivirus(self):
        self.active_app = "AntiVirus"
        if input("🛡️ [1] Scan [2] Clean > ") == "1":
            print("Clean!")
        self.active_app = None
    ...
    def execute_command(self, cmd_str, is_sudo=False):
        ...
        if cmd == "1191":
            ...
            if len(parts) < 2:
                return self._print_buf("Usage: sudo <command>")
            ...
            if len(parts) > 1:
                filepath = parts[1]
                if not filepath.endswith('.dsh') and cmd == "run":
                    filepath += ".dsh"
    ...
    def render_desktop(self):
        ...
        # New Feature: Dynamic Monitor Resolution Check
        if self.user_data.get("dynamic_resolution", True):
            cols, lines = shutil.get_terminal_size(fallback=(80, 60))
            cols = max(50, cols)  # Prevent UI from completely breaking on tiny screens
            lines = max(15, lines)
        else:
            cols, lines = 80, 60
        ...
        avail = max(5, lines - 8)
        if len(self.screen_buffer) > avail:
            self.screen_buffer = self.screen_buffer[-avail:]

        for line in self.screen_buffer:
            print(f"{bg_col}{line}\033[K")
        for _ in range(max(0, avail - len(self.screen_buffer))):
    ...
    def run(self):
        while self.power_on:
            ...
            # ---> THE CHANGES WERE ADDED RIGHT HERE <---
            elif self.current_state == "os":
                self.display_boot_banner()
                self.run_os_loop()
