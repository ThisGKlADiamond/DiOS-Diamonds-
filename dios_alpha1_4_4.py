...
try:
    ...
    # NEW IN 1.4.4: Added run, zip, unzip, paint, bank, alias, autostart, notifications
...
THEMES = {
    ...
    "dark": "\033[30m",
    "edu_gold": "\033[33m",
    "gold": "\033[1;33m",  # NEW IN 1.4.4: Premium theme colors
    "retro": "\033[1;34m"
}
...
WALLPAPERS = {
    ...
    "flowers": "\033[42m",
    "rain": "\033[44m",
    "gold_grid": "\033[43m",  # NEW IN 1.4.4
    "retro_grid": "\033[44m"  # NEW IN 1.4.4
}
...
class DiOS:
    def __init__(self):
        ...
        self.version = "alpha 1.4.4"  # NEW IN 1.4.4
        ...
        self.active_app = None  # NEW IN 1.4.4: For dynamic taskbar
...
alias         : Create command shortcuts (NEW!)
autostart     : Set app on login (NEW!)
...
run <file>: Run .dsh scripts easily (NEW!)
...
zip <dir>   : Archive folder (NEW!)
unzip <file>: Extract archive (NEW!)
...
"""
    ...
    def set_notify(self, text):
        ...
        # NEW IN 1.4.4: Save to Notification Center
    ...
    def run_install_register(self):
        ...
        self.users_db[name] = {
            ...
            "taskbar_color": "neon",
            "installed_apps": [],
            "coins": 50,  # Bonus for 1.4.4
            "aliases": {},  # NEW IN 1.4.4
            "autostart": "",  # NEW IN 1.4.4
            "themes_owned": ["cyber", "neon", "matrix", "hacker", "standard", "dark"],  # NEW IN 1.4.4
            "inbox": [{"id": 1, "from": "DiOS Team", "subject": f"Welcome to {self.version}!",
                       "body": "Welcome to DiOS! Check out the Notification Center and new Education OS features.",
                       "read": False}]
        }
    ...
    def login(self):
        while True:
            ...
            if name in self.users_db and pwd == self.users_db[name]["password"]:
                ...
                # Migrations for older accounts (1.4.3 -> 1.4.4)
                ...
                # NEW IN 1.4.4: Autostart feature execution
                ...
                self.user_data = {"name": "root", "coins": 0, "installed_apps": [], "wallpaper": "black", "aliases": {},
                                  "autostart": ""}
    ...
    def run_med_os_loop(self):
        ...
        while self.current_state == "med_os":
            ...
            if cmd == "1345":
                ...
                print("diagnose       - Diagnose patient symptoms [NEW]")
                print("pharmacy       - Buy medical supplies for clinic [NEW]")
                print("1345           - Exit Med OS")
            elif cmd == "diagnose":
                # NEW IN 1.4.4: Simulate diagnosis for coins
                symptoms = {
    ...
    def run_edu_os_loop(self):
        ...
        # NEW IN 1.4.4: Logic & Algorithms tasks
        logic_tasks = [
        ...
        while self.current_state == "edu_os":
            ...
            print(f"Student: {self.user} | Coins: {self.user_data.get('coins', 0)}")
            ...
            print(f"{THEMES['neon']}[11] Logic & Algorithms (NEW!){THEMES['standard']}")
            print(f"{THEMES['cyber']}[12] Mini-Games (Fun Learning!){THEMES['standard']}")
            ...
            if cmd == "0":
                ...
                if l_cmd.isdigit() and 1 <= int(l_cmd) <= len(langs):
                    ...
                    # NEW IN 1.4.4: Audio-visual simulation
                    print(f"\n[AUDIO SIMULATION] Listen closely to '{word_en}'...")
                    if HAS_WINDOWS_API:
                        for char in word_en:
                            # Generate freq based on char ascii value
                ...
                icon = "📂" if os.path.isdir(os.path.join(self.current_dir, item)) else \
                    "🐍" if item.endswith('.py') else \
                        "📜" if item.endswith('.dsh') else \
                            "📦" if item.endswith('.zip') else "📄"  # NEW IN 1.4.4: zip icon
    ...
    def play_hide_seek(self):
        ...
        try:
            self._print_buf("Starting 3D Environment... (Simulated)")
            time.sleep(1)
    ...
    def run_store(self):
        ...
        apps = {
            "1": ["Construction Sim", "construction", 0, "app"],
            "2": ["Hide In Seek 3D", "hide_seek", 0, "app"],
            "3": ["Number Guesser", "number_guess", 0, "app"],
            "4": ["Golden Theme (Premium)", "gold", 200, "theme"],  # NEW IN 1.4.4
            "5": ["Retro 80s Theme", "retro", 150, "theme"]  # NEW IN 1.4.4
        }
        ...
        choice = input("\nSelect ID to install/buy (or Q): ").strip()
        if choice in apps:
            item = apps[choice]
    ...
    def run_browser(self):
        ...
        while True:
            ...
            if url == 'q':
                ...
                self.set_notify("Browser minimized"); break
            elif url:
                print("Loading...")
        self.active_app = None
    ...
    def run_calculator(self):
        ...
        while True:
            ...
            if expr == 'q':
                ...
                self.set_notify("Calculator minimized"); break
            try:
    ...
    def execute_command(self, cmd_str, is_sudo=False):
        ...
        # NEW IN 1.4.4: Process Aliases
        aliases = self.user_data.get("aliases", {})
        if cmd_raw in aliases:
            # Reconstruct command with alias substitution
        ...
        elif cmd in ["sh", "run"]:  # NEW IN 1.4.4: added run alias explicitly
            if len(parts) > 1:
                filepath = parts[1]
        ...
        # NEW IN 1.4.4: Zip / Unzip simulation
        elif cmd == "zip":
            if len(parts) > 1:
                target = parts[1]
        ...
        # NEW IN 1.4.4: Autostart configuration
        elif cmd == "autostart":
            if len(parts) > 1:
                prog = " ".join(parts[1:])
        ...
        # NEW IN 1.4.4: Notification Center
            ...
            if not notifs:
                ...
                for n in notifs[:15]: print(n)  # Show last 15
            print("=" * 40)
            input("Press Enter to return...")
            ...
            self.battery = 100; self.set_notify("Battery fully charged ⚡")
        elif cmd == "time":
            ...
            if len(parts) > 1:
                try:
                    os.chdir(parts[1]); self.current_dir = os.getcwd(); self._print_buf(
            ...
            if len(parts) > 1:
                try:
                    os.mkdir(parts[1]); self._print_buf(f"Directory '{parts[1]}' created.")
                except Exception as e:
                    self._print_buf(f"Error: {e}")
            else:
            ...
            if len(parts) > 1:
                ...
                if target in ["dios_sys.json", "dios_sys_backup.json"] and not is_sudo and self.current_user != "root":
                    ...
                    try:
                        shutil.rmtree(target) if os.path.isdir(target) else os.remove(target); self._print_buf(
                            f"Deleted: {target}")
                    except Exception as e:
                        self._print_buf(f"Error: {e}")
            else:
    ...
    def render_desktop(self):
        ...
        # NEW IN 1.4.4: Dynamic Taskbar indicating active App
