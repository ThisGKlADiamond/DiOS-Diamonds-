...
try:
    ...
    COMMANDS = ['menu', 'help', 'clear', 'settings', 'store', 'files', 'browser', 'calc', 'messages',
                ...
                'charge', 'lock', 'time', 'ping', 'wget', 'ps', 'kill', 'coins']
...
# This ensures text doesn't have a black highlight by matching text bg to screen bg
WALLPAPERS = {
    "black": "\033[40m",
    "blue": "\033[44m",
    "magenta": "\033[45m",
...
class DiOS:
    def __init__(self):
        ...
        self.user_data = {
            ...
            "password": "",
            "theme": "standard",
            "wallpaper": "black",  # Changed from bg_color for image support
            "taskbar_color": "cyber",
            "installed_apps": [],
            "coins": 0  # New: Diamond Coins Currency
        }
...
coins     : Check Diamond Coins balance (NEW)
...
wget <url>  : Download file from network (NEW)
...
number_guess : Number Guesser Game (Earn Coins)
"""

    def set_4_3_resolution(self):
        # Force terminal aspect ratio to 4:3 (80 columns, 60 lines)
    ...
    def boot_sequence(self, target_state="os"):
        ...
        self._clear_screen()
        print(f"{THEMES['cyber']}--- TGK BIOS v1.0.5 ---{THEMES['standard']}")
    ...
    def run_settings(self):
        while True:
            ...
            if c == 'q':
                ...
                print("1: Solid Black")
                print("2: 33f02bb445a11f1aeae9e34197fb5d1_1.jpg (Spring Meadow)")
                print("3: 23ae770445a11f1a16896dc08cef86a_2.jpg (Rainy Street)")
                print("4: ОСнова.png (Base Purple)")
                bg = input("Select Image > ")
                bg_map = {
                    "1": "black",
                    "2": "33f02bb445a11f1aeae9e34197fb5d1_1.jpg",
                    "3": "23ae770445a11f1a16896dc08cef86a_2.jpg",
                    "4": "ОСнова.png"
                }
                if bg in bg_map:
                    self.user_data["wallpaper"] = bg_map[bg]
                    self.save_system()
                    self.set_notify("Wallpaper updated (Applied OS Background Mode)")
...
app.run()"""
    ...
    def render_desktop(self):
        self._clear_screen()
        # Fallback dimensions matching 4:3 console (80x60 usually, using 80x24 logic for text limits)
        cols = 80
        lines = 60

        # Unified background color using ANSI to match wallpapers
        bg_col = WALLPAPERS.get(self.user_data.get("wallpaper", "black"), "\033[40m")
        reset_col = "\033[0m"

        # Apply background color to entire top bar without black gaps
        print(f"{bg_col}{self.theme_prefix}═" * cols)
        print(f"{bg_col}{self.theme_prefix}" + f" DiOS {self.version} ".center(cols, " "))
        print(f"{bg_col}{self.theme_prefix}═" * cols)

        avail = lines - 8  # Reserved lines for header and taskbar
        if len(self.screen_buffer) > avail: self.screen_buffer = self.screen_buffer[-avail:]

        # Render buffer over colored background without black highlights
        for line in self.screen_buffer:
            print(f"{bg_col}{line}\033[K")  # \033[K clears rest of line with bg color

        # Fill rest of empty space with wallpaper color
    ...
    def run_os_loop(self):
        ...
        while self.current_state == "os":
            self.render_desktop()
            # Ensure input line also matches the background color to avoid black highlight
            bg_col = WALLPAPERS.get(self.user_data.get("wallpaper", "black"), "\033[40m")
            cmd_str = input(f"{bg_col}{self.theme_prefix}Command > {reset_col}").strip()
