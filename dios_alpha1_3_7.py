...
class DiOS:
    def __init__(self):
        ...
        self.user_data = {
            ...
            "installed_apps": [],
            "coins": 0,
            "inbox": [{"id": 1, "from": "DiOS Team", "subject": "Welcome to 1.3.7!",
                       "body": "Welcome to DiOS alpha 1.3.7.\nEnjoy the new Education OS, SSH Terminal, and moryak.exe!",
                       "read": False}]
        }
...
"""
    ...
    def run_edu_os_loop(self):
        # Инструкция: Для добавления новых языков, просто добавляй их в этот словарь
        foreign_languages = {
            "Russian": {"Привет": "Hello", "Яблоко": "Apple", "Школа": "School"}
            # "Spanish": {"Hola": "Hello", "Manzana": "Apple", "Escuela": "School"} # Пример
        }
        ...
        while self.current_state == "edu_os":
            ...
            print("[1] English (Native)      [2] Foreign Language")
            ...
            if cmd == "0":
                ...
                print("Task: Correct the grammar.")
                ans = input("Question: 'He don't like apples.' -> Correct version: ").strip().lower()
                if ans == "he doesn't like apples" or ans == "he does not like apples":
                    print("Correct! +10 Coins");
                    self.user_data["coins"] += 10
                else:
                    print("Incorrect. It should be 'He doesn't like apples.'")
                input("Press Enter...")
            elif cmd == "2":
                print("--- Foreign Languages ---")
                langs = list(foreign_languages.keys())
                for i, l in enumerate(langs): print(f"[{i + 1}] {l}")
                l_cmd = input("Select language: ")
                ...
                print("--- Physics ---")
                ans = input("What is the formula for force? (F = ?) ").strip().upper()
                if ans == "M*A" or ans == "MA":
                    print("Correct! Newton's Second Law. +10 Coins");
                    self.user_data["coins"] += 10
                else:
                    print("Incorrect. F = m*a")
                input("Press Enter...")
            elif cmd == "4":
                print("--- Chemistry ---")
                ans = input("What is the chemical symbol for Gold? ").strip().capitalize()
                if ans == "Au":
                    print("Correct! +10 Coins");
                    self.user_data["coins"] += 10
                else:
                    print("Incorrect. It is Au.")
                input("Press Enter...")
            elif cmd in ["5", "6", "7", "8"]:
                print("Reading materials loaded for this subject. Interactive quizzes coming in next update!")
...
app.run()"""
    ...
    def run_store(self):
        self._clear_screen()
        print(f"💎 DIAMOND STORE | Balance: {self.user_data['coins']} Coins")
        apps = {
