...
class DiOS:
    def __init__(self):
        ...
        self.version = "alpha 1.4.6"  # UPDATED VERSION
        ...
        while True:
            ...
            if not in_secret_store:
                ...
                if not self.user_data.get("magic_door_bought", False):
                    apps["6"] = ["Красная Дверь (Magic)", "magic_door", 10, "item"]
                apps["7"] = ["Оранжевая Дверь", "orange_door", 12, "item"]
                apps["8"] = ["Желтая Дверь", "yellow_door", 14, "item"]
                apps["9"] = ["Зеленая Дверь", "green_door", 16, "item"]
                apps["10"] = ["Голубая Дверь", "blue_door", 18, "item"]
                apps["11"] = ["Синяя Дверь", "indigo_door", 20, "item"]
                apps["12"] = ["Фиолетовая Дверь (Expensive)", "violet_door", 36, "item"]
            else:
                # Secret Menu
                apps["1"] = ["Villager hero", "villager_hero", 50, "item"]
                apps["2"] = ["Колодец желаний", "wishing_well", 0, "item"]
                apps["3"] = ["Смайлик сердечко", "heart_emoji", 15, "item"]
