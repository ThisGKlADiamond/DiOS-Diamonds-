...
"""
    ...
    def boot_sequence(self):
        ...
        # Очищаем экран перед запуском оболочки
        if os.name == 'nt':
            os.system('cls')
        else:
            os.system('clear')
    ...
    def run(self):
        ...
        while self.running:
            try:
                ...
                print("\nUse 'shutdown' to safely turn off DiOS.")
    ...
    def execute_command(self, command):
        ...
        if cmd == "help":
            ...
            self.boot_sequence()
            print(f"Welcome back to DiOS {self.version}! Type 'help' for available commands.")
        ...
        # --- File System Commands ---
            ...
            if len(parts) > 2:
                try:
                    ...
                    # Для копирования папок можно в будущем добавить shutil.copytree
            ...
            if len(parts) > 1:
                try:
                    ...
                    print(f"cat: {parts[1]}: No such file")
                except IsADirectoryError:
                    print(f"cat: {parts[1]}: Is a directory")
                except UnicodeDecodeError:
                    print(f"cat: {parts[1]}: Cannot read binary or non-UTF-8 file")
                except Exception as e:
                    print(f"cat: error: {e}")
            else:
