...
--- Apps & Games ---
antivirus : launch DiOS Antivirus System App
...
"""
    ...
    def save_system_data(self):
        try:
            ...
            print(f"\n[System Warning] Failed to save system data: {e}")
            print("Make sure the system has write permissions in this folder.")
            time.sleep(2)
    ...
    def first_time_setup(self):
        ...
        print("Account created successfully! Rebooting system...\n")
        time.sleep(1.5)
    ...
    def boot_sequence(self):
        ...
        time.sleep(0.3)
        print("Loading Antivirus background shields... Active.")
        time.sleep(0.2)
    ...
    def run_antivirus(self):
        """Интерфейс антивируса DiOS"""
        self._clear_screen()
        print("=" * 50)
        print("🛡️  DiOS Antivirus System App 🛡️")
        print("=" * 50)
        print("Status: Protected")
        print("Last updated: Just now\n")

        while True:
            print("[1] Scan system for viruses")
            print("[2] Clear system cache & temporary files")
            print("[3] Exit Antivirus")

            choice = input("\nSelect an action (1-3): ").strip()

            if choice == "1":
                print("\nInitiating Deep System Scan...")
                folders = ["/sys/kernel", "/usr/bin", "/etc/config", "/home/" + self.user, "/tmp/cache"]
                for folder in folders:
                    print(f"Scanning {folder}...", end="", flush=True)
                    time.sleep(random.uniform(0.3, 0.8))
                    print(" Clean.")

                print("\n✅ Scan Complete! No viruses or malware detected.")
                ...
                time.sleep(1)
                cache_size = random.randint(15, 120)
                print(f"Found {cache_size} MB of temporary files and cache.")
                print("Clearing files...", end="", flush=True)
                time.sleep(1.5)
                print(" Done!")
                print(f"✅ Successfully freed {cache_size} MB of space.")
                input("\nPress Enter to continue...")
                self._clear_screen()
                print("🛡️  DiOS Antivirus System App 🛡️\n")

            elif choice == "3":
                print("Closing Antivirus... Stay safe!")
                time.sleep(0.5)
                self._clear_screen()
                break
            else:
...
if __name__ == "__main__":
    try:
        ...
        # Восстанавливаем цвета терминала и делаем ПАУЗУ, чтобы окно не "вылетало"
        if os.name == 'nt':
            os.system('color 07')
        else:
            print('\033[0m', end='')

        # Вот эта строка спасает от "вылета" консольного окна:
        input("\n[The program has completed its work. Press Enter to close the window...]")